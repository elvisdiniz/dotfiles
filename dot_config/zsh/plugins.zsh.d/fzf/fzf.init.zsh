...
show_file_or_dir_preview="if [ -d {} ]; then ${show_dir_preview} | head -200; else ${show_file_preview}; fi"

export FZF_CTRL_T_OPTS="--preview '$show_file_or_dir_preview'"
export FZF_ALT_C_OPTS="--preview '${show_dir_preview} | head -200'"
...
_fzf_comprun() {
  ...
  case "$command" in
  cd)
    fzf --preview "$( ${show_dir_preview} | head -200)" "$@"
    ;;
  export | unset)
